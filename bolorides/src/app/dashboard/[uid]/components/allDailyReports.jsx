...
// Reliable report type detection
const getReportType = (report) => {
  if (report.rentalRateAmount !== undefined || report.numberOfRentalDays !== undefined) return 'revenue';
  if (report.driverIncome !== undefined || report.carExpense !== undefined) return 'expenses';
            ...
            if (report.balanceAmount > 0 && report.paidAmount < report.amountDue) {
              return (
                <p key={report.id} className="text-red-500 mt-2">
                  Transaction done on {report.transactionDate}. A balance amount of {formatNumber(report.balanceAmount)} is still pending.
                </p>
              );
            }
            return null;
                ...
                {reports.map(report => (
                  <tr key={report.id}>
                    ...
                    <td className="border border-gray-300 p-2">
                      <a
                        href={`/dashboard/${userId}/edit-report?reportId=${report.id}&type=${getReportType(report)}`}
                        target="_blank"
